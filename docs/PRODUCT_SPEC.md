...
At rest, Crisp shows a persistent app shell: Today, Later, and Completed navigation on the left (or its responsive equivalent), with the task workspace on the right. One tap starts a session in that same workspace. Recording never opens a route, screen, modal, or dark-mode replacement. The bottom recorder gains a live audio meter; temporary numbered cards enter at the top of the existing task area in a loose zigzag. There is no finish ritual. With the live relay configured, a natural pause finalizes and interprets one thought while the microphone remains open; without it, Crisp uses the retained-file fallback and briefly resumes after processing. A longer quiet moment commits all draft cards directly to permanent tasks with brief `N added` feedback. Several tasks in one thought must become separate cards. Corrections such as “replace Raju with Rakesh” update the matching active-session card. There is no review, save, confirmation, chatbot, project selection, or priority selection.
...
The flow, wording, and component architecture are the same across platforms. Platform-specific code may handle audio capture, permission, files, safe areas, pointer behavior, and haptics only. Sarvam’s server-side voice activity detection finalizes live thoughts without closing the microphone; the retained-file fallback uses a pause of roughly 1.1 seconds and bounds a continuous turn at roughly 12 seconds. Thirty seconds of quiet commits the session without a countdown. Errors are inline, calm, actionable, and preserve session drafts when possible—never modal, red-text-heavy, or destructive. A stopped retained recording may be recovered for retry after restart. Invisible anonymous auth may protect a configured live relay, but user-visible authentication and optional sync must not block first use.
